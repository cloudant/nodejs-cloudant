...
class CloudantClient {
  ...
  _addPlugins(plugins) {
    ...
    var defaultCfg = {
      retryDelayMultiplier: DEFAULTS.retryDelayMultiplier,
      retryInitialDelayMsecs: DEFAULTS.retryInitialDelayMsecs
    };
    ...
    plugins.forEach(function(p) {
      ...
      if (typeof p === 'function') {
        debug(`Found custom plugin: '${p.id}'`);
        Plugin = p;
        cfg = {};
      } else {
        var plugin = {};

        // expected: { 'pluginName': { 'configKey1': 'configValue1', ... } }
        if (typeof p !== 'object') {
          plugin[p] = {};
        } else {
          plugin = p;
      ...
      if (self._pluginIds.indexOf(Plugin.id) !== -1) {
        ...
        self._plugins.push(
          // instantiate plugin
          new Plugin(self._client, Object.assign({}, defaultCfg, cfg))
        );
