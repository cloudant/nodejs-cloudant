...
class CloudantClient {
  ...
  stopTokenRefresh() {
    ...
    self._plugins.forEach(function(plugin) {
      if (plugin.id === 'cookieauth' || plugin.id === 'iamauth') {
        plugin._tokenManager.stopAutoRenew();
      }
    });
  }
  ...
  _addPlugins(plugins) {
    ...
    plugins.forEach(function(plugin) {
      ...
      if (self._pluginIds.indexOf(Plugin.id) !== -1) {
        ...
        self._plugins.push(
          // instantiate plugin
          new Plugin(self._client, Object.assign({ serverUrl: creds.outUrl }, cfg), self.requestDefaults)
        );
