...
// alias
CloudantClient.prototype.addPlugin = CloudantClient.prototype.addPlugins;

/**
 * Remove plugins from this Cloudant client.
 *
 * @param {Object[]} plugins - Array of plugins to remove.
 */
CloudantClient.prototype.removePlugins = function(plugins) {
  var self = this;
  ...
  plugins.forEach(function(plugin) {
    ...
    let i = self._pluginIds.indexOf(plugin);
    if (i !== -1) {
      self._plugins.splice(i, 1);
      self._pluginIds.splice(i, 1);
    } else {
      debug(`No such plugin found: '${plugin}'.`);
    }
  });
};

// alias
CloudantClient.prototype.removePlugin = CloudantClient.prototype.removePlugins;
