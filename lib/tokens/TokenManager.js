...
class TokenManager {
  ...
  _autoRenew(defaultMaxAgeSecs) {
    ...
    this._renew().then((response) => {
      ...
      clearTimeout(this._autoRenewTimer);
      this._autoRenewTimer = setTimeout(this._autoRenew.bind(this), delayMSecs);
    }).catch((error) => {
      debug(`Failed to auto renew token - ${error}. Retrying in 60 seconds.`);
      clearTimeout(this._autoRenewTimer);
      this._autoRenewTimer = setTimeout(this._autoRenew.bind(this), 60000);
    });
  }
  ...
  stopAutoRenew() {
    debug('Stop auto token renewal process');
    clearTimeout(this._autoRenewTimer);
    this._closeRequest();
  }
}
