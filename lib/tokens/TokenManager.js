...
class TokenManager {
  constructor(client, jar, sessionUrl) {
    ...
    this._requestDefaults = {
      // Configure an agent with only a single socket and no keep-alive for
      // token requests
      agent: new (this._sessionUrl.startsWith('https') ? require('https') : require('http')).Agent({
        keepAlive: false,
        maxSockets: 1
      }),
      headers: {
    ...
    // Configure the client with the defaults if possible
    this._internalClient = (client.defaults) ? client.defaults(this._requestDefaults) : client;
  ...
  // A legacy plugin used as a client is not a request object and therefore does
  // not have a "defaults" function. In those cases we need to apply the defaults
  // we require for token requests to the passed in options.
  get _client() {
    return (this._internalClient.defaults) ? this._internalClient : function(opts, cb) {
      this._internalClient(Object.assign(opts, this._requesDefaults), cb);
    };
  }
  ...
  _renew() {
    if (!this._isTokenRenewing) {
      ...
      let boundedTokenReceived = this._tokenReceived.bind({
        tokenManager: this
      });
      this._getToken(boundedTokenReceived);
    }
    return new Promise((resolve, reject) => {
      this._tokenExchangeEE.once('success', function(response) {
        debug('Succeeded to renew token');
        this.tokenManager._isTokenRenewing = false;
        this.tokenManager._tokenExchangeEE.removeAllListeners();
        resolve(response);
      }.bind({tokenManager: this}));
      this._tokenExchangeEE.once('error', function(error, response) {
  ...
  _tokenReceived(error, response) {
    debug('Answer received to token renewal request.');
    if (error) {
      this.tokenManager._tokenExchangeEE.emit('error', error, response);
    } else {
      this.tokenManager._attemptTokenRenewal = false;
      this.tokenManager._tokenExchangeEE.emit('success', response);
    }
  }
  ...
  waitForToken() {
    return new Promise((resolve, reject) => {
      this._tokenExchangeEE.once('success', function(res) {
        resolve(res);
      });
      this._tokenExchangeEE.once('error', function(error, response) {
        error.response = response;
