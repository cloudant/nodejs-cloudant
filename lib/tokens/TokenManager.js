...
class TokenManager {
  ...
  _renew() {
    if (!this._isTokenRenewing) {
      ...
      tokenExchangeEE = new EventEmitter().setMaxListeners(Infinity);
      debug('Starting token renewal.');
      this._getToken((error, response) => {
        if (error) {
          tokenExchangeEE.emit('end', error, response);
        } else {
          tokenExchangeEE.emit('end', null, response);
    ...
    return new Promise((resolve, reject) => {
      tokenExchangeEE.once('end', (error, response) => {
        if (error) {
