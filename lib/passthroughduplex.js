...
class PassThroughDuplex extends stream.Duplex {
  constructor(Writable, Readable, options) {
    ...
    if (typeof Readable === 'undefined' && typeof options === 'undefined') {
      options = Writable;

      // defaults
      Writable = stream.PassThrough;
      Readable = stream.PassThrough;
    }

    this.destinations = [];

    this.passThroughReadable = new Readable();
    this.passThroughReadable.on('error', function(error) {
      this.emit(error);
    });

    this.passThroughWritable = new Writable();
