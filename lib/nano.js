...
module.exports = exports = nano = function dbScope(cfg, couchdb_callback) {
  var serverScope = {};
  ...
  function relax(opts, callback) {
    ...
    if (typeof opts.qs === 'object' && !_.isEmpty(opts.qs)) {
      ['startkey', 'endkey', 'key', 'keys', 'sort'].forEach(function(key) {
        if (key in opts.qs) {
  ...
  function docModule(dbName) {
    ...
    cloudant.db_functions(docScope, relax);
    return docScope;
  }
  ...
  serverScope = cloudant.server_functions(serverScope);

  // If the user specified a callback to the database module, try to ping
  // the server.
  if (couchdb_callback)
    ping_server(function(er, pong, headers) {
      if (er)
        ...
        couchdb_callback(null, serverScope, pong, headers);
    });
